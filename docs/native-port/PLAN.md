...
## 10. Milestone B2 spec: the local runner (decided 2026-09-03 from the benchmark)

Engine: MLX Qwen3-ASR as a managed sidecar. whisper.cpp was measured at 1.8 s for the reference clip against 0.40 s (0.6B) and 0.98 s (1.7B) for Qwen, so it is not the primary engine. The sidecar needs a Python 3.10+ interpreter on the user's Mac (Homebrew or python.org); the app detects one, never bundles one, and says plainly what to install when none is found.

Pieces:

- `runner/runner.py`, shipped inside the app bundle (`Contents/Resources/runner/`). Standard library only plus `mlx-audio`: a `ThreadingHTTPServer` on `127.0.0.1:<port>` serving `GET /health` (state: loading | ready | unloaded, model id, resident bytes), `POST /prewarm` (load if unloaded, return immediately), `POST /v1/audio/transcriptions` (multipart `file`, `model`, `language`; `prompt` accepted and ignored; one inference at a time behind a lock; response `{"text": ...}` like the LAN bridge), and idle unload after `--idle-minutes` (default 10) so a quiet app holds no model memory. The port and the model id come from argv. The server binds loopback only and refuses any other bind address by construction.
- Core `LocalRunner` (`crates/openflow-core/src/runner.rs`): finds the interpreter, creates the venv under the app dir on first use (`Install` step, about 600 MB, on demand with progress), downloads the chosen model with the Hugging Face hub client inside the venv (`Download` step, on demand with progress; models `mlx-community/Qwen3-ASR-1.7B-8bit` default "accurate", `mlx-community/Qwen3-ASR-0.6B-8bit` "fast"), spawns the sidecar with a free port, polls `/health` for readiness, restarts on crash with backoff (max three in a minute, then reports failed), kills it on quit. Exposes state to the UI and `prewarm()` for the engine to call when recording starts, so the load overlaps the user speaking.
- Settings: `transcription_backend` = `remote` | `local` (default `remote`), `local_model` = `accurate` | `fast`, `local_idle_minutes` (default 10), `local_only` (bool, default false). When `local`, the engine transcribes against `Custom("http://127.0.0.1:<port>/v1")` with no key and applies the dictionary as a deterministic post-pass, since Qwen ignores the prompt. The post-pass ports the iPhone `DictionaryPostPass` rules exactly (whole word, longest entry first, case-insensitive with dictionary casing, sentence-initial capitalisation only for all-lowercase entries, single pass, `heard -> Correct` arrow form) with the same test vectors, and runs for every backend, since it is idempotent and Whisper output benefits too.
- `local_only` guard: `transcribe::client()` builds a client whose every request is checked against a loopback-only rule (host is `127.0.0.1`, `::1`, or `localhost`); a hosted URL is refused before any connection with an error naming the setting. Unit test proves a hosted URL is refused and a loopback URL passes. While `local_only` is on, cleanup and voice are disabled in the UI unless their provider is loopback, and the pipeline enforces the same rule regardless of UI.
- Native Settings, Providers tab: a "Transcription runs" choice, Online provider / On this Mac. The On-this-Mac panel shows runner state, the model picker with the measured cost next to each option (1.7B about 2.5 GB resident, 0.98 s; 0.6B about 1.0 GB, 0.40 s, weaker on names), Install and Download buttons with progress, idle-unload minutes, the Local only toggle with one sentence on what it disables. Onboarding gets an "On this Mac (private)" card that saves `transcription_backend = local` and sends the user to that panel. General tab gets "Live preview while recording" bound to `live_preview` with the cost sentence from its doc.
- Tauri host: keeps compiling against core; no UI for the runner there (Milestone C retires it).
