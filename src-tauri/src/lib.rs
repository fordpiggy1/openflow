...
async fn stop_recording_and_transcribe(state: State<'_, AppState>) -> Result<Transcription, String> {
    ...
    let raw_text = transcribe::transcribe_audio(wav_bytes, &transcription_key, language.as_deref(), &transcription_provider, stt_model.as_deref()).await?;
    ...
    let formatted = if format_enabled {
        ...
        match transcribe::format_text(&raw_text, &fmt_key, None, &fmt_provider, chat_model.as_deref()).await {
            Ok(text) => text,
...
fn handle_hotkey_release(app: &AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let raw_text = match transcribe::transcribe_audio(wav_bytes, &api_key, language.as_deref(), &transcription_provider, stt_model.as_deref()).await {
            Ok(t) => t,
            Err(e) => {
                let _ = app_handle.emit("transcription-error", &e);
                let _ = app_handle.emit("recording-state", "idle");
        ...
        let formatted = if format_enabled {
            let (fmt_provider, fmt_key) = if same_provider {
                (transcription_provider.clone(), api_key.clone())
            } else {
                let fp = state.db.get_setting("formatting_provider").unwrap_or(provider_str.clone());
                let fk = state.db.get_setting("formatting_api_key").unwrap_or(api_key.clone());
                (Provider::from_str(&fp), fk)
            };
            match transcribe::format_text(&raw_text, &fmt_key, None, &fmt_provider, chat_model.as_deref()).await {
                Ok(text) => text,
